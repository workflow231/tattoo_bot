...
def test_styles_keyboard_paginates_after_ten_items() -> None:
    styles = [Style(id=index, name=f"Категория {index}") for index in range(1, 12)]

    keyboard = build_styles_reply_keyboard(styles=styles, page=0)
    texts = _keyboard_texts(keyboard)

    assert "Категория 1" in texts
    assert "Категория 10" in texts
    assert "Категория 11" not in texts
...
def test_styles_keyboard_second_page_has_previous_button() -> None:
    ...
    assert "Категория 11" in texts
    assert "Категория 1" not in texts
...
def test_sketches_keyboard_paginates_after_ten_items() -> None:
    sketches = [
        Sketch(id=index, name=f"Услуга {index}", price=None, style_id=1)
    ...
    assert "Услуга 1 — цена договорная" in texts
    assert "Услуга 10 — цена договорная" in texts
    assert "Услуга 11 — цена договорная" not in texts
    assert NEXT_PAGE_BUTTON in texts


def test_duplicate_service_buttons_are_unique_on_same_page() -> None:
    sketches = [
        Sketch(id=1, name="Покрытие", price=None, style_id=1),
        Sketch(id=2, name="Покрытие", price=None, style_id=1),
    ]

    keyboard = build_sketches_reply_keyboard(sketches=sketches, page=0)
    texts = _keyboard_texts(keyboard)

    assert "Покрытие — цена договорная (#1)" in texts
    assert "Покрытие — цена договорная (#2)" in texts


def test_duplicate_service_buttons_map_to_stable_ids() -> None:
    sketches = [
        Sketch(id=1, name="Покрытие", price=None, style_id=1),
        Sketch(id=2, name="Покрытие", price=None, style_id=1),
    ]

    assert build_sketch_button_text_by_id(sketches=sketches) == {
        1: "Покрытие — цена договорная (#1)",
        2: "Покрытие — цена договорная (#2)",
    }
...
def test_admin_sketch_actions_keyboard_shows_management_buttons() -> None:
    texts = _keyboard_texts(build_admin_sketch_actions_keyboard(simple_bot=False))
...
def test_admin_sketch_actions_keyboard_hides_categories_in_simple_mode() -> None:
    texts = _keyboard_texts(build_admin_sketch_actions_keyboard(simple_bot=True))

    assert ADD_SKETCH_BUTTON in texts
    assert DELETE_STYLE_BUTTON not in texts
    assert EDIT_STYLE_BUTTON not in texts
    assert DELETE_SKETCH_BUTTON in texts
    assert EDIT_SKETCH_BUTTON in texts


def test_admin_sketch_edit_fields_keyboard_shows_editable_fields() -> None:
    texts = _keyboard_texts(build_admin_sketch_edit_fields_keyboard(simple_bot=False))
...
def test_admin_sketch_edit_fields_keyboard_hides_category_in_simple_mode() -> None:
    texts = _keyboard_texts(build_admin_sketch_edit_fields_keyboard(simple_bot=True))

    assert EDIT_SKETCH_NAME_BUTTON in texts
    assert EDIT_SKETCH_STYLE_BUTTON not in texts
